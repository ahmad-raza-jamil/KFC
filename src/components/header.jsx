import React from 'react'
import '../custom_css/header.css'
export default function Header({cartCounter}) {
    return (
        <>
        <div className='container'>
            <nav className="navbar navbar-dark  navbar-bg-color navbar-expand-lg bg-body-tertiary">
                <div className="container-fluid">
                    <a className="navbar-brand" href="#home"><img src="data:image/png;base64," className="logo" alt="Company Logo" />
                    </a>
                    <button className="navbar-toggler" type="button" data-bs-toggle="collapse" data-bs-target="#navbarSupportedContent" aria-controls="navbarSupportedContent" aria-expanded="false" aria-label="Toggle navigation">
                    <span className="navbar-toggler-icon custom-toggler-icon" />
                    </button>
                    <div className="collapse navbar-collapse" id="navbarSupportedContent">
                        <ul className="navbar-nav me-auto mb-2 mb-lg-0">
                            <li className="nav-item">
                                <a className="nav-link active" aria-current="page" href="#home"><button className='btn btn-1'> <img className="delivery" src="https://www.kfcpakistan.com/static/media/Delivery.793adbaa710baf75ba78.png" alt="Delivery" />
                                <span className='delivery-text'>Delivery</span></button></a>
                            </li>
                            <li className="nav-item">
                                <a className="nav-link" href="#home"><button className='btn btn-2'> <img className="delivery" src="https://www.kfcpakistan.com/static/media/pickup.47bb874f83ea6acb5a47.png" alt="Delivery" />
                                <span className='delivery-text'>Pickup</span></button></a>
                            </li>
                        </ul>
                        <form className="d-flex" role="search">
                        
                        <button className="fly-cart-btn" type="button" data-bs-toggle="offcanvas" data-bs-target="#staticBackdrop" aria-controls="staticBackdrop"><b>{cartCounter}</b></button>
                        
                            <button className="btn login-btn" type="submit">Login</button>
                        </form>
                        
                    </div>
                </div>
            </nav>
            </div>
        </>
    )
}
